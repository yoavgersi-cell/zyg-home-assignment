...
DETECTED CHANGE: ${detectedChange}${
  ...
  try {
    ...
    // Two attempts covering transient network errors, API 5xx and malformed
    // output alike - the fallback only engages after both genuinely fail.
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        console.error(`[agent/generate] attempt ${attempt + 1} failed:`, attemptErr);
        lastErr = attemptErr;
      }
    }
    throw lastErr instanceof Error ? lastErr : new Error('Live generation failed');
  } catch (err) {
